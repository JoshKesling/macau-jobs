class Cv < ApplicationRecord
  ...
  validates_attachment_content_type :head_pic, content_type: %w[image/jpg image/jpeg image/png image/gif], size: { in: 0..1500.kilobytes }, if: Proc.new {|a| a.head_pic.present? }
  has_attached_file :body_pic, styles: { thumb: '100x100', small: '300x300', large: '600x600' }
  validates_attachment_content_type :body_pic, content_type: %w[image/jpg image/jpeg image/png image/gif], size: { in: 0..1500.kilobytes }, if: Proc.new {|a| a.body_pic.present? }
  ...
  validates :visa_exp_date, timeliness: { on_or_after: lambda { Date.current } }, allow_blank: true
  validates :passport_country, length: { maximum: 50 }, allow_blank: true
  validates :passport_number, length: { in: 5..25 }, allow_blank: true
  validates :passport_expiration_date, timeliness: { on_or_after: lambda { Date.current } }, allow_blank: true
